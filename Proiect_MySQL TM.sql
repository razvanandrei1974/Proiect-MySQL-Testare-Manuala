...
## Am modificat denumirea coloanei din Observatii in Categorie

alter table Patrimoniu 
change Observatii Categorie varchar(20);

## Am vizualizat tabelul Patrimoniu
select * from Patrimoniu;


## Am introdus date in tabelul Patrimoniu
...
## Am modificat numarul de carcatere text in coloana Denumire

alter table Patrimoniu
modify Denumire varchar(30);

## Am vizualizat datele din tabelul Patrimoniu
select * from Patrimoniu;

# Am corectat greseala de la randul 15 din tabel
UPDATE Patrimoniu
...
## Am mai introdus doua randuri in tabelul Patrimoniu
...
## Am interogat cate inregistrari avem in tabel 
SELECT COUNT(*)
FROM Patrimoniu;

## Am interogat cate ID-uri au valoarea mai mare de 2500 

SELECT COUNT(Valoare)
FROM Patrimoniu
WHERE Valoare > 2500;

## Am calculat valoarea totala a coloanei "Valoare" din tabel

SELECT SUM(Valoare) AS total
FROM Patrimoniu;


## Am centralizat datele din tabelul Patrimoniu dupa Categorie
SELECT COUNT(ID), Categorie
FROM Patrimoniu
GROUP BY Categorie;

## Am corectat greselile de scriere din coloana Categorie

UPDATE Patrimoniu
SET Categorie = 'Mobilier'
WHERE ID = 60;

ALTER TABLE Patrimoniu AUTO_INCREMENT=120;
...
## Stergem tabelul AchizitiPublice

drop table AchizitiiPublice;

## Am recreat tabelul AchizitiiPublice
create table AchizitiiPublice
(
ID int not null,
Denumire varchar(30),
...
## Introducem date in tabelul AchizitiiPublice
...
## Adaugam in tabelul Patrimoniu achizitiile facute cu comanda Insert Into
...
## Am comparat coloana Denumire cu comanda Inner Join din tabelele Patrimoniu si AchizitiiPublice
...
## Am comparat coloana Valoare cu comanda Inner Join din tabelele Patrimoniu si AchizitiiPublice
SELECT Patrimoniu.Valoare, Patrimoniu.Valoare
FROM AchizitiiPublice
INNER JOIN Patrimoniu ON Patrimoniu.Valoare = AchizitiiPublice.Valoare;


## Am comparat coloana Valoare cu comanda Left Join din tabelele Patrimoniu si AchizitiiPublice
SELECT Patrimoniu.Valoare, Patrimoniu.Valoare
FROM AchizitiiPublice
LEFT JOIN Patrimoniu ON Patrimoniu.Valoare = AchizitiiPublice.Valoare;

## Am comparat coloana Valoare cu comanda Right Join din tabelele Patrimoniu si AchizitiiPublice
SELECT Patrimoniu.Valoare, Patrimoniu.Valoare
FROM AchizitiiPublice
RIGHT JOIN Patrimoniu ON Patrimoniu.Valoare = AchizitiiPublice.Valoare;

## Am comparat coloana Denumire cu comanda Right Join din tabelele Patrimoniu si AchizitiiPublice
SELECT Patrimoniu.Denumire, Patrimoniu.Denumire
FROM AchizitiiPublice
RIGHT JOIN Patrimoniu ON Patrimoniu.Denumire = AchizitiiPublice.Denumire;


## Am comparat cu comanda Cross Join tabelele Patrimoniu si AchizitiiPublice
SELECT Patrimoniu.Denumire, Patrimoniu.Denumire
FROM AchizitiiPublice
...
ALTER TABLE AchizitiiPublice
ADD CONSTRAINT AchizitiiPublice
FOREIGN KEY (ID) REFERENCES Patrimoniu(ID);

# Am interogat tabelul Patrimoniul pentru a vedea in care departamnete se regaseste produsul "scaun Papilon" .
SELECT * FROM Patrimoniu
WHERE Denumire = 'scaun papilon' OR departament = '*';

# Am interogat tabelul Patrimoniul pentru a vedea in care departamente se regasesc  produse din categoria Echipamente IT
SELECT * FROM Patrimoniu
WHERE Categorie = 'Echipamente IT' OR departament = '*';

# Am interogat tabelul Patrimoniul pentru a vedea in care departamente se regaseste produsul "Imprimanta Canon".
SELECT * FROM Patrimoniu
WHERE Denumire = 'Imprimanta Canon' OR departament = '*';

# Am interogat tabelul Patrimoniul pentru a vedea in care departament nu se regasesc produse din categoria "Mobilier".
SELECT * FROM Patrimoniu
WHERE NOT Categorie = 'Mobilier';

# Am selectat din tabelul Patrimoniu coloanele Denumire, Departament si Valoare
SELECT Denumire, Departament, Valoare
FROM Patrimoniu;

# Am interogat tabelul Patrimoniu pentru a afla care produse incep cu litera "i" .
SELECT * FROM patrimoniu
WHERE Denumire LIKE 'i%';

# Am interogat tabelul Patrimoniu pentru a afla care produse incep cu litera "d".
SELECT * FROM patrimoniu
WHERE Denumire LIKE 'd%';

# Am interogat tabelul Patrimoniu pentru a afla media coloanei valoare.
SELECT AVG(Valoare)
FROM Patrimoniu;

# Am interogat tabelul Patrimoniu pentru a afla media coloanei valoare la produsele cu o valoare mai mare sau egala cu 100.
SELECT AVG(Valoare)
FROM Patrimoniu;
...
## Am calculat valoarea totala a coloanei "Valoare"
SELECT SUM(Valoare)
FROM Patrimoniu;
